...
- (id)initWithItems:(NSArray*)items parentView:(UIView *)p delegate:(id<BlurMenuDelegate>)d {
    ...
    if (self) {
        ...
        [blurEffectView setFrame:CGRectMake(0,0, self.parent.frame.size.width, self.parent.frame.size.height)];
        ...
        CGSize logoSize = CGSizeMake(240, 60);
        UIImageView *logo = [[UIImageView alloc] initWithFrame:CGRectMake((self.frame.size.width - logoSize.width)/2.0, 100.0, logoSize.width, logoSize.height)];
